...
export function handleTransfer(event: Transfer): void {
  ...
  if (!factory) {
    log.error('Factory not found (handleTransfer)', [FACTORY_ADDRESS]);
    return;
  }
  ...
  if (!pair) {
    log.error('Pair not found (handleTransfer)', [event.address.toHexString()]);
    return;
  }
  ...
  if (event.params.to.toHexString() == ADDRESS_ZERO && event.params.from.toHexString() == pair.id) {
    ...
    if (burns.length > 0) {
      ...
      if (!currentBurn) {
        log.error('BurnEvent not found (handleTransfer)', [burns[burns.length - 1]]);
        return;
      }
    ...
    if (mints.length !== 0 && !isCompleteMint(mints[mints.length - 1])) {
      ...
      if (!mint) {
        log.error('MintEvent not found (handleTransfer)', [mints[mints.length - 1]]);
        return;
      }
      burn.feeTo = mint.to;
...
export function handleSync(event: Sync): void {
  ...
  if (!pair) {
    log.error('Pair not found (handleSync)', [event.address.toHex()]);
    return;
  }
  let token0 = Token.load(pair.token0);
  if (!token0) {
    log.error('Token 0 not found (handleSync)', [pair.token0]);
    return;
  }
  let token1 = Token.load(pair.token1);
  if (!token1) {
    log.error('Token 1 not found (handleSync)', [pair.token1]);
    return;
  }
  let uniswap = DexFactory.load(FACTORY_ADDRESS);
  if (!uniswap) {
    log.error('DexFactory not found (handleSync)', [FACTORY_ADDRESS]);
    return;
  }
  ...
  if (!bundle) {
    log.error('Bundle not found (handleSync)', ['1']);
    return;
  }
...
export function handleMint(event: Mint): void {
  ...
  if (!transaction) {
    log.error('Transaction not found (handleMint)', [event.transaction.hash.toHexString()]);
    return;
  }
  let mints = transaction.mints;
  let mint = MintEvent.load(mints[mints.length - 1]);
  if (!mint) {
    log.error('MintEvent not found (handleMint)', [mints[mints.length - 1]]);
    return;
  }

  let pair = Pair.load(event.address.toHex());
  if (!pair) {
    log.error('Pair not found (handleMint)', [event.address.toHex()]);
    return;
  }
  let token0 = Token.load(pair.token0);
  if (!token0) {
    log.error('Token 0 not found (handleMint)', [pair.token0]);
    return;
  }
  let token1 = Token.load(pair.token1);
  if (!token1) {
    log.error('Token 1 not found (handleMint)', [pair.token1]);
    return;
  }
  let uniswap = DexFactory.load(FACTORY_ADDRESS);
  if (!uniswap) {
    log.error('DexFactory not found (handleMint)', [FACTORY_ADDRESS]);
    return;
  }
  ...
  let bundle = Bundle.load('1');
  if (!bundle) {
    log.error('Bundle not found (handleMint)', ['1']);
    return;
  }
...
export function handleBurn(event: Burn): void {
  let transaction = Transaction.load(event.transaction.hash.toHexString());
  if (!transaction) {
    log.error('Transaction not found (handleBurn)', [event.transaction.hash.toHexString()]);
    return;
  }
  ...
  if (!burn) {
    log.error('BurnEvent not found (handleBurn)', [burns[burns.length - 1]]);
    return;
  }

  let pair = Pair.load(event.address.toHex());
  if (!pair) {
    log.error('Pair not found (handleBurn)', [event.address.toHex()]);
    return;
  }
  let token0 = Token.load(pair.token0);
  if (!token0) {
    log.error('Token 0 not found (handleBurn)', [pair.token0]);
    return;
  }
  let token1 = Token.load(pair.token1);
  if (!token1) {
    log.error('Token 1 not found (handleBurn)', [pair.token1]);
    return;
  }
  let uniswap = DexFactory.load(FACTORY_ADDRESS);
  if (!uniswap) {
    log.error('DexFactory not found (handleBurn)', [FACTORY_ADDRESS]);
    return;
  }
  ...
  let bundle = Bundle.load('1');
  if (!bundle) {
    log.error('Bundle not found (handleBurn)', ['1']);
    return;
  }
  let amountTotalUSD = token1.derivedETH
    .times(token1Amount)
...
export function handleSwap(event: Swap): void {
  let pair = Pair.load(event.address.toHexString());
  if (!pair) {
    log.error('Pair not found (handleSwap)', [event.address.toHex()]);
    return;
  }
  let token0 = Token.load(pair.token0);
  if (!token0) {
    log.error('Token 0 not found (handleSwap)', [pair.token0]);
    return;
  }
  let token1 = Token.load(pair.token1);
  if (!token1) {
    log.error('Token 1 not found (handleSwap)', [pair.token1]);
    return;
  }
  ...
  let bundle = Bundle.load('1');
  if (!bundle) {
    log.error('Bundle not found (handleSwap)', ['1']);
    return;
  }
  ...
  let uniswap = DexFactory.load(FACTORY_ADDRESS);
  if (!uniswap) {
    log.error('DexFactory not found (handleSwap)', [FACTORY_ADDRESS]);
    return;
  }
